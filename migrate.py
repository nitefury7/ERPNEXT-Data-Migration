...
def fetch_document_list(doctype, cookies):
    ...
    response = requests.get(f"{config.SOURCE_URL}/api/resource/{doctype}", cookies=cookies)
    if response.status_code == 200:
...
def send_data(doctype, name, data):
    """Send a specific document's data to the destination Frappe instance."""
    response = requests.post(
        f"{config.DESTINATION_URL}/api/resource/{doctype}/{name}",
        headers=destination_headers,
        json={"data": data},
    )
    if response.status_code == 200:
        print(f"Successfully sent record: {name}")
    else:
        print(
            f"Failed to send record: {name}. Status Code: {response.status_code}, Response: {response.text}"
        )
...
if __name__ == "__main__":
    ...
    if names:
        ...
        for name in names:
            ...
            # if data:
            #     # print(data)
            #     send_data(doctype=config.doctype, name=name, data=data)
            # else:
